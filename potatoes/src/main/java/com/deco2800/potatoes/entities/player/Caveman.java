...
public class Caveman extends Player {
	...
	/* Caveman Animations */
	private Map<Direction, TimeAnimation> cavemanWalkAnimations = makePlayerAnimation("caveman", PlayerState.walk, 8,
			750, this::walkCompletionHandler);
	private Map<Direction, TimeAnimation> cavemanIdleAnimations = makePlayerAnimation("caveman", PlayerState.idle, 1, 1,
			null);
	private Map<Direction, TimeAnimation> cavemanDamagedAnimations = makePlayerAnimation("caveman", PlayerState.damaged,
			1, 200, this::damagedCompletionHandler);
	private Map<Direction, TimeAnimation> cavemanDeathAnimations = makePlayerAnimation("caveman", PlayerState.death, 3,
			300, this::completionHandler);
	private Map<Direction, TimeAnimation> cavemanAttackAnimations = makePlayerAnimation("caveman", PlayerState.attack,
			5, 200, this::completionHandler);
	private Map<Direction, TimeAnimation> cavemanInteractAnimations = makePlayerAnimation("caveman",
			PlayerState.interact, 5, 400, this::completionHandler);
			...
			if (target.isPresent()) {
				...
				GameManager.get().getWorld()
						.addEntity(new PlayerProjectile(target.get().getClass(), new Vector3(pPosX, pPosY, pPosZ),
								new Vector3(targetPosX, targetPosY, 0), 8f, 100, Projectile.ProjectileTexture.CHILLI,
								null, null, this.getDirection().toString(),
